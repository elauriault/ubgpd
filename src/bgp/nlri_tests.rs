...
fn test_nlri_ipv4_valid() {
    ...
    let nlri = NlriBuilder::default().net(IpNet::V4(net)).build().unwrap();
...
fn test_nlri_ipv6_valid() {
    ...
    let nlri = NlriBuilder::default().net(IpNet::V6(net)).build().unwrap();
...
fn test_nlri_equality_valid() {
    ...
    let nlri1 = NlriBuilder::default().net(IpNet::V4(net)).build().unwrap();

    let nlri2 = NlriBuilder::default().net(IpNet::V4(net)).build().unwrap();
...
fn test_nlri_from_invalid_ipv4_prefix_length_invalid() {
    ...
    assert!(
        result.is_err(),
        "Should return error on invalid IPv4 prefix length"
    );
}
...
fn test_nlri_from_invalid_ipv6_prefix_length_invalid() {
    ...
    assert!(
        result.is_err(),
        "Should return error on invalid IPv6 prefix length"
    );
}
...
fn test_nlri_from_insufficient_ipv4_octets_invalid() {
    ...
    assert!(
        result.is_err(),
        "Should return error on insufficient octets"
    );
}
...
fn test_mpnlri_inconsistent_nexthop_length_invalid() {
    ...
    assert!(
        result.is_err(),
        "Should return error on inconsistent nexthop length"
    );
}
